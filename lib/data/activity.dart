...
Future<void> makeconsumeActivity(EntrySaved entry, List<int> chapter, ReadType type) async {
  final Activity? last = await isar.activitys
      .filter()
      .typeEqualTo('consume')
      .sortByEndDesc()
      .findFirst();
  ...
  if (last != null) {
    final dynamic data = json.decode(last.data);
  ...
  final Activity newact = Activity.init();
  newact.type = 'consume';
  newact.begin = now;
  newact.end = now;
