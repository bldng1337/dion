    ...
    return SelectionArea(
      child: Bionify(
        ...
        markStyle: TextStyle(
          ...
          color: Theme.of(context).textTheme.displaySmall?.color,
          fontWeight:
              stringToFontWeight(TextReaderSettings.bionichighlight.value),
        ),
      ),
    );
  }
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          actions: [
            ...
            IconButton(
              icon: Icon(
                source?.getEpdata().isBookmarked ?? false
                    ? Icons.bookmark
  ...
  Widget build(BuildContext context) {
    double edge = (MediaQuery.of(context).size.width -
            ((TextReaderSettings.textwidth.value / 100) *
                MediaQuery.of(context).size.width)) /
        2;
    if (TextReaderSettings.adaptivewidth.value && isVertical(context)) {
      edge = 0.0;
    }
    return CallbackShortcuts(
      bindings: {
        ...
        const SingleActivator(LogicalKeyboardKey.arrowLeft): navPreviousChapter,
        const SingleActivator(LogicalKeyboardKey.keyW): scrollUp,
        const SingleActivator(LogicalKeyboardKey.keyS): scrollDown,
        const SingleActivator(LogicalKeyboardKey.keyE): bookmark,
        const SingleActivator(LogicalKeyboardKey.keyQ): openwebview,
