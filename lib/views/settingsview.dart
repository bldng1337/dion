...
final settingspage = SettingPageBuilder("Settings", [
  SettingsNavTile("Text Reader", "Settings for the PlainText Reader", textreadersettings,
      icon: Icons.menu_book_outlined),
  const SettingsNavTile("Mangareader", "Settings for Mangareader", mangareadersettings,
      icon: Icons.broken_image),
  const SettingsNavTile("Videoplayer", "Settings for the Videoplayer", videoplayersettings,
      icon: Icons.video_camera_back_rounded),
  const SettingsNavTile("Sync", "Settings for Syncronisation", syncsettings,
      icon: Icons.broken_image),
  SettingsNavTile("Library", "Settings for Library", librarysettings,
...
final librarysettings = SettingPageBuilder("Manga Reader Settings", [
  ...
  const BooleanTile("Filter Media Type", "Filter by media type",
      LibrarySettings.shouldfiltermediatype,
      icon: Icons.filter_alt_sharp),
  ConditionalTile(
      LibrarySettings.shouldfiltermediatype,
      SimpleChoiceTile("Media Type Filter", "By which media type to filter",
          LibrarySettings.filtermediatype,
          choices: MediaType.values
              .map((e) => e.toString().replaceFirst("MediaType.", ""))
              .toList())),
  const BooleanTile(
      "Filter Status", "Filter by status", LibrarySettings.shouldfilterstatus,
      icon: Icons.filter_alt_sharp),
  ConditionalTile(
      LibrarySettings.shouldfilterstatus,
      SimpleChoiceTile("Filter Status", "By which status to filter",
          LibrarySettings.filtermediatype,
...
const SettingPageBuilder videoplayersettings =
  ...
  BooleanTile("Auto Subtitle", "Auto Select a Subtitle if its available",
      VideoplayerSetting.autousesubtitle),
  ConditionalTile(
    VideoplayerSetting.autousesubtitle,
    LanguageTile("Subtitle", "Subtitle that gets auto selected if available",
...
const SettingPageBuilder mangareadersettings =
  ...
  DoubleTile("Image width", "Width of Images", MangareaderSetting.imagewidth,
      icon: Icons.width_full,min: 20,max: 100),
  BooleanTile("Adaptive Width", "Turns Text width off on vertical orientation",
...
final SettingPageBuilder textreadersettings =
  ...
  const SimpleChoiceTile(
      "ReaderChoice", "Which Reader you want to use", TextReaderSettings.reader,
      choices: ["Paginated", "Infinityscroll"], icon: Icons.book),
  SimpleChoiceTile(
      "TextWeight", "Textweight of characters", TextReaderSettings.textweight,
      choices: FontWeight.values
          .map((e) => e.toString().replaceFirst("FontWeight.", ""))
          .toSet()
          .toList(),
      icon: Icons.highlight),
  const DoubleTile("Textsize", "Size of the Text", TextReaderSettings.textsize,
      icon: Icons.format_size_outlined,min: 1,max:50),
  const DoubleTile("TextWidth", "Width of a Text line in Percent",
  ...
  ConditionalTile(
      TextReaderSettings.bionic,
      SimpleChoiceTile("BionicWeight", "Textweight of characters highlighted",
